                ...
                <div class="grid">
                    <div class="col-12 md:col-6">
                        <div class="mb-4">
                            ...
                            <div class="space-y-1">
                                <p><strong>Client:</strong> {{ repairDetails.clientName || 'N/A' }}</p>
                        ...
                        <div class="mb-4">
                            <h5 class="mb-2">Mechanic Responsible:</h5>
                            <p>{{ repairDetails.mechanicName || 'N/A' }}</p>
                        </div>
...
export class RepairDetailsManagerComponent implements OnInit {
    ...
    loadRepairDetails(): void {
        if (this.repairId) {
            this.apiRoutes.getReparationById(this.repairId).subscribe({
                next: (data: any) => {
                    console.log('Received repair data:', data); // Ajoutez cette ligne pour inspecter les données
    ...
    transformRepairDetailsData(data: any): any {
        ...
        const vehicleId = depositId.vehicleId || {};
        const clientId = vehicleId.clientId || {};
    
        return {
            ...data,
            clientName: `${clientId.firstName || 'N/A'} ${clientId.lastName || 'N/A'}`,
            vehicleRegistration: vehicleId.registrationNumber || 'N/A',
            vehicleBrand: vehicleId.brandId?.brand || 'N/A',
            vehicleModel: vehicleId.model || 'N/A',
            ...
            mechanicName: data.mechanics?.map((m: any) => `${m.firstName} ${m.lastName}`).join(', ') || 'N/A' // Correction ici
        };
    }
    ...
    backToRepairsList(): void {
        this.router.navigate(['/reparation/reparation-manager-list']);
    }
}
